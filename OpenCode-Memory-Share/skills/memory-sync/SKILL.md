...
### 1.1 加载历史消息
```bash
node C:/Users/12527/.config/opencode/load-memory-context.js --load
```

### 1.2 读取历史文件
```bash
type C:\Users\12527\.config\opencode\.memory-history.txt
```
...
```bash
node C:/Users/12527/.config/opencode/load-memory-context.js --sync user "用户的完整消息内容"
```

### 2.2 AI回复完成后 → 立即上传AI回复
当你完成对用户的回复后，在回复末尾追加执行：
```bash
node C:/Users/12527/.config/opencode/load-memory-context.js --sync assistant "你的完整回复内容摘要"
```
